...
class History:
    ...
    def addPart(self,partid:int,orientation:int,img:np.array)->None:
        if len(self.history[(self.history['partid']==partid) & (self.history['orientation']==orientation)].index) == 0:
            ...
            self.history.loc[(self.history['partid']==partid) & (self.history['orientation']==orientation),'image'] = [img]
            self.history.loc[(self.history['partid']==partid) & (self.history['orientation']==orientation),'classification'] = np.NAN
            self.history.loc[(self.history['partid']==partid) & (self.history['orientation']==orientation),'metadata'] = np.NAN
    ...
    def classifyPart(self,partid:int,orientation:int,classification:int,metadata)->None:
        self.history.loc[(self.history.partid==partid) & (self.history.orientation==orientation),'classification'] = classification
        self.history.loc[(self.history.partid==partid) & (self.history.orientation==orientation),'metadata'] = metadata
